...
public class PreferencesParser : IPreferencesParser
{
    ...
    public PreferencesParser(IFileSystem fileSystem, IDiscoveredSettingRegistry registry)
    {
    ...
    public async Task<BrowserSettings> ParseAsync(string filePath)
    {
        ...
        {
            throw new PreferencesFileNotFoundException($"Preferences file not found at: {filePath}");
        }
        ...
        {
            throw new ProfileAccessDeniedException($"Access denied to preferences file: {filePath}", ex);
        }
        catch (IOException ex)
        {
            throw new CorruptedProfileException($"Failed to read preferences file: {filePath}. {ex.Message}");
        }
        ...
        try
        {
            ...
            if (rootNode == null)
            {
                throw new InvalidPreferencesJsonException("Preferences file is empty.", null!);
            }
        }
        catch (JsonException ex)
        {
            throw new InvalidPreferencesJsonException("Failed to parse Preferences JSON.", ex);
        }

        var traverser = new JsonPathTraverser(_registry, "Brave", "Preferences");
